...
const deleteTransfers = `-- name: DeleteTransfers :one
DELETE FROM transfers WHERE id = $1 RETURNING id, from_account_id, to_account_id, amount, created_at
`

func (q *Queries) DeleteTransfers(ctx context.Context, id int64) (Transfers, error) {
	row := q.db.QueryRowContext(ctx, deleteTransfers, id)
	var i Transfers
	err := row.Scan(
		&i.ID,
		&i.FromAccountID,
